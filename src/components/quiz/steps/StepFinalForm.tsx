  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-muted/30 rounded-lg p-3 border border-border">
        <p className="text-xs text-muted-foreground mb-1">{t('quiz.final.yourData')}</p>
      ...
      >
        <label className="block text-xs font-medium text-foreground mb-1.5">
          {t('quiz.final.message')}
        </label>
        <div className="relative">
          <MessageSquare className="absolute left-3 top-2.5 w-4 h-4 text-muted-foreground" />
          <Textarea
            value={state.message}
            onChange={(e) => setFormField('message', e.target.value.replace(/[<>]/g, ''))}
            placeholder={t('quiz.final.messagePlaceholder')}
      ...
      <p className="text-center text-xs text-muted-foreground">
        {t('quiz.final.privacy')}{' '}
        <a href="#" className="text-[#f06800] hover:underline">{t('quiz.final.privacyLink')}</a>
      </p>
    </div>
  );
}
