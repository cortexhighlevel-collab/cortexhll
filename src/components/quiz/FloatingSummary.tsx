    ...
    <AnimatePresence>
      ...
      >
        <div className="flex items-center gap-2 mb-4">
          ...
          <span className="font-medium text-gray-900">{t('quiz.floating.title')}</span>
        </div>

        {state.service === 'sites' ? (
          <div className="space-y-3">
            <div>
              <p className="text-xs text-gray-500 uppercase tracking-wider mb-1">{t('quiz.floating.setup')}</p>
              <motion.p 
                key={price.setup}
                initial={{ scale: 1.1 }}
                animate={{ scale: 1 }}
            ...
            {price.monthly > 0 && (
              <div className="pt-2 border-t border-gray-100">
                <p className="text-xs text-gray-500 uppercase tracking-wider mb-1">{t('quiz.floating.monthly')}</p>
                <motion.p 
                  key={price.monthly}
                  initial={{ scale: 1.1 }}
                  animate={{ scale: 1 }}
          ...
          <div className="space-y-3">
            <div>
              <p className="text-xs text-gray-500 uppercase tracking-wider mb-1">{t('quiz.floating.monthlyMgmt')}</p>
              <p className="text-3xl font-bold text-gray-900">
                {formatCurrency(TRAFFIC_CONFIG.managementFee)}
              </p>
              <p className="text-xs text-gray-400">{t('quiz.recurring.month')}</p>
            </div>
            
            {state.platforms.length > 0 && (
              <div className="pt-2 border-t border-gray-100">
                <p className="text-xs text-gray-500">
                  {state.platforms.length} {state.platforms.length === 1 ? t('quiz.floating.platforms1') : t('quiz.floating.platformsN')}
                </p>
              </div>
            )}
            ...
            {state.investment && (
              <div className="pt-2 border-t border-gray-100">
                <p className="text-xs text-gray-500">{t('quiz.floating.investment')}</p>
