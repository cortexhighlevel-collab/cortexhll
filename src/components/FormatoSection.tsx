...
const ServiceCard = ({
  ...
  items,
  imageUrl
}: ServiceCardProps) => <div className="course-card group font-dm">
    ...
    <div className="absolute inset-0 p-6 flex flex-col justify-between">
      ...
      <div className="flex flex-col gap-4">
        <div>
          <p className="text-white/80 text-sm mb-1 font-light">{subtitle}</p>
          <h3 className="text-white text-2xl md:text-3xl font-medium leading-tight mb-3">{title}</h3>
          <ul className="space-y-1.5">
            {items.map((item, index) => <li key={index} className="text-white/70 text-sm flex items-center gap-2 font-light">
        ...
        <a href="#contato" className="inline-flex items-center gap-2 px-6 py-3 rounded-full bg-white text-foreground text-sm font-medium uppercase tracking-wider w-fit hover:bg-white/90 transition-colors">
          Saiba Mais
          <ArrowIcon />
        </a>
      </div>
