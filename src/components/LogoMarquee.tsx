const LogoMarquee = () => {
  const logos = [
    ...
    { type: 'text', name: 'TRIDENT' },
    { type: 'text', name: 'WILLBANK' },
    { type: 'svg', name: 'Uber', svg: (
    ...
    { type: 'text', name: 'HOTMART' },
    { type: 'text', name: 'SAMSUNG' },
    { type: 'svg', name: 'Globo', svg: (
      <svg viewBox="0 0 80 24" className="h-6 w-auto">
        <circle cx="12" cy="12" r="10" fill="none" stroke="currentColor" strokeWidth="2"/>
        <circle cx="12" cy="12" r="4" fill="currentColor"/>
      </svg>
    )},
    { type: 'svg', name: 'iFood', svg: (
      <svg viewBox="0 0 80 24" className="h-5 w-auto">
  ...
  const LogoItem = ({ logo }: { logo: typeof logos[0] }) => (
    <div className="logo-item flex items-center justify-center px-6 h-20 grayscale opacity-60 hover:grayscale-0 hover:opacity-100 transition-all duration-300">
      {logo.type === 'svg' ? (
        ...
        <span className="font-bold text-xl tracking-tight text-foreground">{logo.name}</span>
      )}
    </div>
  );
