...
const OfertaSection = () => {
  ...
  const features = [{
    id: "segmentacao", title: t("oferta.f1.title"), description: t("oferta.f1.text"),
    image: ofertaSegmentacao, icon: Target, isNew: true
  }, {
    id: "pricing", title: t("oferta.f2.title"), description: t("oferta.f2.text"),
    image: ofertaPricing, icon: LineChart, isNew: true
  }, {
    id: "agent-monitoramento", title: t("oferta.f3.title"), description: t("oferta.f3.text"),
    image: ofertaTurnaround, icon: Bot, isNew: true
  }, {
    id: "otimizacao", title: t("oferta.f4.title"), description: t("oferta.f4.text"),
    image: ofertaLaunch1, icon: BarChart3
  }, {
    id: "relatorios", title: t("oferta.f5.title"), description: t("oferta.f5.text"),
    image: ofertaLaunch2, icon: Headphones
  }];
        ...
        <motion.div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-6 mb-12" variants={fadeUp}>
          <div className="max-w-2xl">
            <div className="flex items-center gap-3 mb-4">
              ...
              <p className="text-sm font-medium text-orange-400 uppercase tracking-widest">{t("oferta.badge")}</p>
            </div>
            <h2 className="text-3xl sm:text-4xl md:text-5xl font-medium tracking-tight">{t("oferta.title")}</h2>
            <p className="mt-4 text-base md:text-lg text-white/60 leading-relaxed">{t("oferta.subtitle")}</p>
          </div>
        </motion.div>
            ...
            <div className="p-6 sm:p-8">
              <div className="flex items-center gap-3 mb-4">
                <span className="inline-flex items-center gap-1.5 rounded-full bg-gradient-to-r from-orange-500/20 to-orange-600/10 border border-orange-500/30 px-3 py-1 text-[11px] font-semibold text-orange-400 uppercase tracking-wider">
                  <span className="w-1.5 h-1.5 rounded-full bg-orange-400" />{t("oferta.results.tag")}
                </span>
                <span className="text-sm text-white/50">{t("oferta.results.roas")}</span>
              </div>
              <h3 className="text-2xl sm:text-3xl font-medium tracking-tight leading-tight">{t("oferta.mainCard.title")}</h3>
              <p className="mt-3 text-sm sm:text-base text-white/60 leading-relaxed max-w-xl">{t("oferta.mainCard.text")}</p>
              <div className="mt-6 flex flex-wrap items-center gap-4">
                <a href="#cases" className="group/btn inline-flex items-center gap-2 rounded-xl border border-white/10 bg-white/5 px-5 py-2.5 text-sm font-medium text-white/90 backdrop-blur hover:bg-white/10 hover:border-white/20 transition-all duration-300">
                  <FileText className="h-4 w-4 text-white/60 group-hover/btn:text-white transition-colors" />{t("oferta.viewResults")}
                </a>
                <CTAButton href="#contato" icon={<ArrowRight className="h-4 w-4" />} className="nav-cta">{t("oferta.requestProposal")}</CTAButton>
              </div>
                  ...
                  <div className="relative flex items-end justify-between gap-6">
                    <div className="flex-1 relative h-24 sm:h-28">
                      <div className="absolute inset-0 flex flex-col justify-between opacity-20">
                        {[...Array(4)].map((_, i) => (<div key={i} className="border-t border-white/30 border-dashed" />))}
                      </div>
                      <svg className="absolute inset-0 w-full h-full overflow-visible" preserveAspectRatio="none" viewBox="0 0 200 110">
                        ...
                        <path d="M0 95 Q20 90, 30 85 T60 75 T90 65 T120 50 T150 35 T180 25 T195 15 L195 110 L0 110 Z" fill="url(#chartGradientFill)" />
                        <path d="M0 95 Q20 90, 30 85 T60 75 T90 65 T120 50 T150 35 T180 25 T195 15" fill="none" stroke="url(#lineGradientStroke)" strokeWidth="3" strokeLinecap="round" filter="url(#glowEffect)" />
                      ...
                      <div className="absolute inset-0 flex items-end justify-between px-2">
                        {[15, 20, 25, 35, 45, 60, 75, 85, 95].map((h, i) => (
                          <motion.div key={i} className="w-1.5 sm:w-2 rounded-full bg-gradient-to-t from-orange-500/0 via-orange-500/50 to-orange-400" style={{ height: `${h}%` }} initial={{ height: 0 }} whileInView={{ height: `${h}%` }} transition={{ delay: 0.1 * i, duration: 0.5, ease: "easeOut" }} />
                        ))}
                      </div>
                    </div>
                    <div className="relative flex-shrink-0 p-4 sm:p-5 rounded-2xl bg-gradient-to-br from-white/15 via-white/5 to-transparent backdrop-blur-xl border border-white/30" style={{ boxShadow: '0 8px 32px rgba(240, 104, 0, 0.3), inset 0 1px 0 rgba(255,255,255,0.2), inset 0 -1px 0 rgba(0,0,0,0.1)' }}>
                      ...
                      <div className="relative text-center">
                        <div className="flex items-center justify-center gap-1 mb-1">
                          <motion.svg className="w-4 h-4 sm:w-5 sm:h-5 text-green-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" initial={{ y: 5, opacity: 0 }} whileInView={{ y: 0, opacity: 1 }} transition={{ delay: 0.5, duration: 0.4 }}>
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M7 11l5-5m0 0l5 5m-5-5v12" />
                          </motion.svg>
                          <span className="text-[10px] sm:text-xs text-green-400 font-medium uppercase tracking-wider">ROI</span>
                        </div>
                        <motion.div className="text-2xl sm:text-3xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-orange-300 via-orange-400 to-orange-500" initial={{ scale: 0.8, opacity: 0 }} whileInView={{ scale: 1, opacity: 1 }} transition={{ delay: 0.4, duration: 0.5, type: "spring" }}>
                          +145,68%
                        </motion.div>
                        <span className="text-[10px] text-white/50">{t("oferta.monthlyAvg")}</span>
                      </div>
                    </div>
                  </div>
                </div>
            ...
            return <motion.div key={feature.id} className="group relative overflow-hidden rounded-3xl border border-white/10 bg-[#141414] hover:border-white/20 hover:bg-[#1a1a1a] transition-all duration-500" variants={fadeUp}>
              <div className="p-5 sm:p-6">
                <div className="flex items-start justify-between mb-4">
                  <div className="p-2.5 rounded-xl bg-gradient-to-br from-orange-500/20 to-orange-600/5 border border-orange-500/20"><Icon className="h-5 w-5 text-orange-400" /></div>
                  {feature.isNew && <span className="inline-flex items-center rounded-full bg-gradient-to-r from-orange-500/20 to-orange-600/10 border border-orange-500/30 px-2.5 py-0.5 text-[10px] font-semibold text-orange-400 uppercase tracking-wider">{t("oferta.new")}</span>}
                </div>
            ...
            return <motion.div key={feature.id} className="group relative overflow-hidden rounded-3xl border border-white/10 bg-[#141414] hover:border-white/20 hover:bg-[#1a1a1a] transition-all duration-500" variants={fadeUp}>
              <div className="p-5 sm:p-6 h-full flex flex-col">
                <div className="flex items-center gap-3 mb-3">
                  <div className="p-2 rounded-lg bg-gradient-to-br from-orange-500/20 to-orange-600/5 border border-orange-500/20"><Icon className="h-4 w-4 text-orange-400" /></div>
                  <h3 className="text-base font-medium tracking-tight">{feature.title}</h3>
                  {feature.isNew && <span className="inline-flex items-center rounded-full bg-gradient-to-r from-orange-500/20 to-orange-600/10 border border-orange-500/30 px-2 py-0.5 text-[9px] font-semibold text-orange-400 uppercase tracking-wider">{t("oferta.new")}</span>}
                </div>
